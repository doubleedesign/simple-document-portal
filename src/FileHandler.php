...
class FileHandler extends FileUploadHandler {
	 ...
	 * @return mixed
	 */
	function format_custom_url($value, $post_id, $field): mixed {
		if(get_post_type($post_id) !== 'document') return $value;
		if($field['name'] !== 'protected_document_file') return $value;
	 ...
	 * @return string
	 */
	function persist_custom_url(string $url, int $attachment_id): string {
		if(!$this->is_attached_to_document($attachment_id)) return $url;
	 ...
	 * @return bool
	 */
	function user_can_access_documents(): bool {
	 ...
	 * @return bool
	 */
	function is_attached_to_document(int $attachment_id): bool {
		$query = new WP_Query([
			'post_type' => 'document',
			'meta_query' => [
				[
	 ...
	 * @return void
	 */
	function filter_document_files_from_media_library(WP_Query $query): void {
