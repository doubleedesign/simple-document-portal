...
class AdminUI {
	...
	public function __construct() {
		...
		add_filter('manage_edit-folder_columns', [$this, 'add_folder_admin_columns'], 10, 1);
		// TODO: Prefix sorting isn't working correctly
		add_filter('manage_edit-folder_sortable_columns', [$this, 'add_folder_admin_columns'], 10, 1);
		add_action('manage_folder_custom_column', [$this, 'populate_folder_admin_columns'], 10, 3);
	...
	public function populate_document_admin_columns($column, $post_id): void {
		...
		if($column === 'document_folder') {
			...
			if(!empty($folders) && !is_wp_error($folders)) {
				echo join(', ', array_map(function(WP_Term $folder) {
	...
	public function add_folder_admin_columns($columns): array {
		$columns['prefix'] = __('Prefix', 'simple-document-portal');

		// Put prefix column after the checkbox
		$columns = array_merge(
			array_slice($columns, 0, 1),
			['prefix' => __('Prefix', 'simple-document-portal')],
	...
	public function populate_folder_admin_columns($value, $column, $term_id) {
		if($column === 'prefix') {
			$prefix = get_term_meta($term_id, 'prefix', true);
